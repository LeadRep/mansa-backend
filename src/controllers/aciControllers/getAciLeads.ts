...
export const getAciLeads = async (req: Request, res: Response) => {
  try {
    const pageParam =
      typeof req.query.page === "string" ? Number.parseInt(req.query.page, 10) : NaN;
    const limitParam =
    ...
    const limitCandidate = Number.isNaN(limitParam) || limitParam < 1 ? DEFAULT_LIMIT : limitParam;
    ...
    const leads = rows.map((lead) => normalizeLead(lead.get({ plain: true }) as PlainLead));
